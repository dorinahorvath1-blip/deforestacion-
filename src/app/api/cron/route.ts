...
async function getCommunitiesWithNewDeforestation(): Promise<string[]> {
  // GEE integration stub — returns empty until service account is configured.
  // To enable: add GOOGLE_SERVICE_ACCOUNT_KEY to Vercel env vars.
  if (!process.env.GOOGLE_SERVICE_ACCOUNT_KEY) {
    console.log("GEE service account not configured. Skipping deforestation check.");
    return [];
  }
  // TODO: implement GEE REST API call here
  return [];
}
...
export async function GET(req: NextRequest) {
  ...
  // Get subscribers for affected communities via Supabase REST API
  const params = communitiesWithAlerts.map(c => `comunidad=eq.${encodeURIComponent(c)}`).join("&");
  const subsRes = await fetch(
    `${SUPABASE_URL}/rest/v1/suscriptores?${params}&activo=eq.true&select=nombre,email,comunidad`,
    {
      headers: {
        "apikey": SUPABASE_KEY,
  ...
  // Send alert emails via Resend REST API
  ...
  return NextResponse.json({
    message: `Alertas enviadas a ${subscribers.length} suscriptores.`,
